...
export default function MyInquiries() {
  ...
  const fetchInquiries = async (contractorCompanyId: string) => {
    try {
      setLoading(true);
      const response = await fetch(`/api/contractor/inquiries?contractorId=${contractorCompanyId}`);
      const data = await response.json();
      ...
      <div className="mobile-container bg-gray-50">
        ...
        <div className="p-4">
          <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 text-center">
            ...
            <Link href="/login" className="text-blue-600 mt-2 inline-block">
              ورود به حساب کاربری
            </Link>
          </div>
        </div>
    ...
    <div className="mobile-container bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-10">
        <div className="flex items-center justify-between p-4">
          ...
          <h1 className="text-lg font-bold">استعلام‌های من</h1>
          <Link href="/contractor/price-inquiry" className="text-blue-600 text-sm">
            + جدید
          </Link>
        </div>
      </div>
      ...
      <div className="p-4 pb-20">
        {inquiries.length === 0 ? (
          <div className="bg-white rounded-lg p-8 text-center">
            ...
            <h3 className="text-lg font-bold mb-2">هنوز استعلامی ارسال نکرده‌اید</h3>
            <p className="text-gray-600 mb-4">برای ارسال استعلام قیمت به تأمین‌کنندگان، روی دکمه "جدید" کلیک کنید</p>
            <Link
              ...
              <div key={inquiry.id} className="bg-white rounded-lg p-4 border border-gray-200">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex-1">
                    <h3 className="font-bold text-lg mb-1">{inquiry.product}</h3>
                        ...
                        >
                              ...
                              {response.supplier ? (
                                <>
                                  <div className="w-8 h-8 bg-blue-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                    {response.supplier.logo || (response.supplier.name && response.supplier.name.length > 0 ? response.supplier.name.charAt(0) : "?")}
                                  </div>
                                  <div>
                                    <p className="font-medium text-sm">{response.supplier.name}</p>
                                  </div>
                                </>
                              ) : (
                                <p className="font-medium text-sm">تأمین‌کننده</p>
                              )}
                            </div>
                          ...
                          <div className="grid grid-cols-2 gap-2 mt-2 text-sm">
                            <div>
                              ...
                              <span className="font-bold text-green-600 mr-1">{response.price}</span>
                            </div>
                            {response.deliveryTime && (
                              <div>
                                <span className="text-gray-600">زمان تحویل:</span>
                                <span className="font-medium mr-1">{response.deliveryTime}</span>
                              </div>
                            )}
                          </div>
                          {response.notes && (
