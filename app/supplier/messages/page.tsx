...
function SupplierMessagesInner() {
        ...
        {conversations.length === 0 ? (
          ...
          <div className="divide-y divide-gray-200">
            {conversations.map((conversation) => (
              <div
                key={conversation.id}
                onClick={() => setSelectedCompanyId(conversation.otherCompanyId)}
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold text-lg">
                    {conversation.otherCompany.logo ? (
                      ...
                      conversation.otherCompany.name && conversation.otherCompany.name.length > 0 ? conversation.otherCompany.name.charAt(0) : "?"
                    )}
                  </div>
...
function ChatView({
  ...
  const [otherCompany, setOtherCompany] = useState<any>(null);
    ...
    try {
      ...
      if (data.messages && data.messages.length > 0) {
        const unreadIds = data.messages
          .filter((m: any) => !m.read && !m.isSender)
          .map((m: any) => m.id);
        ...
        <div className="flex items-center gap-3 p-4">
          ...
          <div className="w-10 h-10 bg-blue-500 rounded-full flex items-center justify-center text-white font-bold">
            {otherCompany?.logo ? (
              ...
              (otherCompany?.name && otherCompany.name.length > 0 ? otherCompany.name.charAt(0) : "?")
            )}
          </div>
