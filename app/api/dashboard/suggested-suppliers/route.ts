...
export async function GET(request: Request) {
  try {
    ...
    const contractorCategories = user.Companies.CompanyCategories.map(c => c.category)
      ...
      const suppliersWithStats = suppliers.map((supplier) => {
        const avgRating =
          supplier.Reviews.length > 0
            ? supplier.Reviews.reduce((sum: number, r: { rating: number }) => sum + r.rating, 0) / supplier.Reviews.length
            : 0
        ...
        return {
          ...
          logo: (supplier.logo 
            ...
            : null) ?? (supplier.name && supplier.name.length > 0 ? supplier.name.charAt(0) : "?"),
    ...
    const suppliersWithMatchScore = suppliers.map((supplier) => {
      const supplierCategories = supplier.CompanyCategories.map(c => c.category)
      
      // تعداد دسته‌بندی‌های مشترک
      const commonCategories = supplierCategories.filter(cat =>
        contractorCategories.some(cc => 
          cc.includes(cat) || cat.includes(cc) || 
      ...
      return {
        ...
        logo: supplier.logo || (supplier.name && supplier.name.length > 0 ? supplier.name.charAt(0) : "?"),
        rating: Math.round(avgRating * 10) / 10,
        verified: supplier.isVerified,
        matchScore,
      }
    ...
    const sortedSuppliers = suppliersWithMatchScore
      ...
      .slice(0, limit)
      .map(({ matchScore, ...supplier }) => supplier) // حذف matchScore از خروجی
