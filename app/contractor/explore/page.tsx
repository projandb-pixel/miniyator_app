...
export default function ContractorExplore() {
  ...
  const handleCreatePost = async () => {
    ...
    console.log("Creating post with:", {
      ...
      imageType: typeof newPostImage,
      imageIsString: typeof newPostImage === "string",
      imageIsNull: newPostImage === null,
      imageLength: newPostImage && typeof newPostImage === "string" ? newPostImage.length : 0,
      imagePreview: newPostImage && typeof newPostImage === "string" ? newPostImage.substring(0, 100) : null,
    });

    // اطمینان از اینکه newPostImage یک string است
    const imageToSend = newPostImage && typeof newPostImage === "string" ? newPostImage : null;
  ...
  const handleLike = async (postId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(errorData.error || "خطا در لایک کردن پست");
      }
    } catch (error) {
        ...
        img.onload = () => {
          ...
          if (ctx) {
            ...
            console.log("Image resized and converted to base64:", {
              length: resizedBase64.length,
              type: typeof resizedBase64,
              isString: typeof resizedBase64 === "string",
              preview: resizedBase64.substring(0, 100),
            });
            // اطمینان از اینکه resizedBase64 یک string است
            if (typeof resizedBase64 === "string" && resizedBase64.length > 0) {
            ...
            if (typeof result === "string" && result.length > 0) {
              ...
              console.error("Reader result is not a valid string");
              alert("خطا در خواندن تصویر. لطفاً دوباره تلاش کنید");
            }
          }
        };
        img.onerror = () => {
          ...
          if (typeof result === "string" && result.length > 0) {
            ...
            console.error("Reader result is not a valid string in fallback");
